...
// Room-code character set — single source of truth (also referenced by shared.js).
...
function aggregateZones(pendingAllocations) {
  const totals = { docks: 0, strip: 0, slums: 0 };
...
function zoneDelta(crew, res, role, classificationOutcome, isHomeTurf = false, crewRecoveryCostEnabled = true) {
  let result;
  if (classificationOutcome === 'all_tied') {
    ...
    switch (role) {
      case 'highest':
      case 'lone_highest':
        result = crewRecoveryCostEnabled ? -(res + 1) : (res === 0 ? 0 : -res); break;
      case 'middle':
        result = 1; break;
      case 'lowest':
      case 'lone_lowest':
...
module.exports = { ZONES, CHARS, PLAYER_ICONS, aggregateZones, classifyZones, zoneDelta, validateAllocation, determineWinners };
